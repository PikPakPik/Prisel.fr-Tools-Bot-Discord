...
module.exports = async (client, oldState, newState) => {
    ...
    con.query(`SELECT gp.color FROM Bot as gp WHERE guildId ='${newState.guild.id}'`, async (e, row) => {
        color = row != null ? row[0].color : '#3A73A0';
        con.query(`SELECT gs.vocalLogs, gs.vocallogsChannel FROM guildSettings as gs WHERE gs.guildId ="${newState.guild.id}"`, async (e, row) => {
...
// -- déplacement
    ...
    } else if(newState.channelID == db.tempo.channel) {
// -- Obtiens la catégorie 
var category = oldState.guild.channels.cache.get(db.tempo.category);
// -- Crée un salon de type vocal avec comme nom le pseudo de la personne et le définis dans la catégorie
oldState.guild.channels.create(`${db.tempo.emoji} ${newState.member.user.username}`, {
...
// -- Ajoute des permissions au salon pour le membre en lui attribuant toutes les permissions sur le salon
c.updateOverwrite(newState.guild.roles.everyone, {
    VIEW_CHANNEL: false, CONNECT:false, MANAGE_CHANNELS: null, MANAGE_ROLES: null,
})

c.updateOverwrite(newState.id, { 
    CONNECT:true, SPEAK:true,
        MEMBER_MOVE:true,VIEW_CHANNEL:true,
...
var str_chan = null;
if(oldState) {
str_chan = oldState.guild.channels.cache.find(c => c.id === vocallogsChannel)
} else if(newState) {
str_chan = newState.guild.channels.cache.find(c => c.name === vocallogsChannel)
}
if(!str_chan) return;
...
if(oldState.streaming === false && newState.streaming) {
    str_chan.send({embed:{ description: `**${newState.member.user.username}**#${newState.member.user.discriminator} (\`${newState.id}\`) partage son écran dans [\`${newState.channel.name}\`](https://discord.com/channels/${newState.guild.id}/${newState.channel.id})`, color: color, author: { name: "📽️ Un stream a commencé" }, footer: { text: `🕙 ${getNow().time}` } }}) 
} else {
    str_chan.send({embed:{ description: `**${newState.member.user.username}**#${newState.member.user.discriminator} (\`${newState.id}\`) ne partage plus son écran dans [\`${newState.channel.name}\`](https://discord.com/channels/${newState.guild.id}/${newState.channel.id})`, color: color, author: { name: "🎞️ Un stream est fini" }, footer: { text: `🕙 ${getNow().time}` } }}) 
...
str_chan.send({embed:{ description: `**${executor.username}**${executor.discriminator} (\`${executor.id}\`) a donné la permissions de d'écouter a **${oldState.member.user.username}**#${oldState.member.user.discriminator} (\`${oldState.id}\`) dans le salon [\`${newState.channel.name}\`](https://discord.com/channels/${newState.guild.id}/${newState.channel.id})`, color: color, author: { name: "🔊 Demute d'un membre" }, footer: { text: `🕙 ${getNow().time}` } }}) 
 }
} else if (oldState.channel === null) {
    str_chan.send({embed:{ description: `**${newState.member.user.username}**#${newState.member.user.discriminator} (\`${newState.id}\`) s'est connecté au salon [\`${newState.channel.name}\`](https://discord.com/channels/${newState.guild.id}/${newState.channel.id})`, color: color, author: { name: "✔️ Connexion" }, footer: { text: `🕙 ${getNow().time}` } }}) 
} else if(newState.channel === null) {
    if(!oldState.channel) return;
    str_chan.send({embed:{ description: `**${oldState.member.user.username}**#${oldState.member.user.discriminator} (\`${oldState.id}\`) a quitté le salon [\`${oldState.channel.name}\`](https://discord.com/channels/${oldState.guild.id}/${oldState.channel.id})`, color: color, author: { name: "❌ Déconnexion" }, footer: { text: `🕙 ${getNow().time}` } }}) 
} else if (oldState.channel !== null && newState.channel !== null) {
